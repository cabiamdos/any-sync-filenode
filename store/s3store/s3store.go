...
func (s *s3store) Init(a *app.App) (err error) {
	...
	// define my variables
	conf.Bucket = os.Getenv("ENV_MINIO_BUCKET")
	conf.IndexBucket = os.Getenv("ENV_MINIO_INDEX_BUCKET")
	conf.Endpoint = os.Getenv("ENV_MINIO_ENDPOINT")
	conf.Credentials.AccessKey = os.Getenv("ENV_MINIO_ACCESS_KEY")
	conf.Credentials.SecretKey = os.Getenv("ENV_MINIO_SECRET_KEY")
	if conf.Profile == "" {
